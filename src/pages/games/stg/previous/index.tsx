...
const PreviousStg = () => {
  ...
  const queryVariables = {
    variables: {
      where: {
        status: {
          _eq: E_Stg_Statuses_Enum.Previous,
        },
      },
    },
  }
  ...
  const NoStgAlert = () => (
    <div className="w-full max-w-md">
      <Alert title="Not found" closeable={false} color="blue">
        <p>There is no previous game to display.</p>
      </Alert>
    </div>
  )

  const previousStg = data?.stgs?.[0] as Stgs
  const orderedResults = calculateWinner(previousStg?.submissions)

  return (
    <>
      <Head>
        <title>skrrrt | previous game</title>
      </Head>
            ...
            <div className="w-full">
              ...
              >
                {orderedResults && orderedResults.length > 0 && (
                  <div className="pb-4">
                    <div className="w-full overflow-auto rounded-lg scrollbar max-h-96 shandow-sm">
                      <StgResults stgId={previousStg?.id} orderedResults={orderedResults} />
                    </div>
                  </div>
                )}
              </Collapse>
            </div>
            {previousStg && previousStg.sets?.length > 0 ? (
              <>
                {displayType === STG_SET_DISPLAY_TYPES.CARD ? (
                  <StgSetsGrid stg={previousStg} />
                ) : (
                  <div className="w-full overflow-auto rounded-lg shadow-sm scrollbar max-h-96">
                    <StgSetsTable stg={previousStg} />
                  </div>
                )}
              </>
            ) : (
